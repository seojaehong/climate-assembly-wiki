...
Generated: 2026-07-04 07:20 KST
...
| Agenda Form name field is required | PASS |
| Agenda Form response count after refresh | 0 |
| Agenda Form unique voter count after refresh | 0 |
| Agenda duplicate handling | `name_latest_response` |
...
| Decision V0/V1A/V1B name fields are required | PASS |
| Decision V0/V1A/V1B response counts after refresh | 0 / 0 / 0 |
| Decision V0/V1A/V1B duplicate handling | `name_latest_response` |
| Decision result JSON generated | PASS |
| Reflection Form group choices | 1~17조 + 연구진 |
...
| Vote completion by attendee count | PARTIAL | Current Forms now collect required names. The system can compare unique voter count to the announced target, but homonyms/manual name variation still require operator judgment. |
| Duplicate vote prevention | PARTIAL | The refresh scripts count only the latest response per identical normalized name. This prevents simple duplicate submissions but is not as strict as verified login or participant tokens. |
...
For today's Google Forms setup, treat completion as "unique voter count reached the announced attendee target and name list looks clean." If strict duplicate blocking is required, choose one of these before opening the vote:

1. Participant token mode: add a required participant code field and deduplicate by code in the refresh script. This is field-friendly and does not require Google login.
2. Google login mode: collect verified email and limit to one response. This is strict but can slow field voting.
