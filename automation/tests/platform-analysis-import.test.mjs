...
import { buildPlatformAnalysisImportPlan } from '../platform-analysis-import.mjs';
...
test('rejects missing citation mappings instead of producing a partial import plan', () => {
  expect(() => buildPlatformAnalysisImportPlan({
    ...
    sourceMappings: [],
  })).toThrow('Missing source mapping: utterance-missing');
});
...
test('rejects duplicate recommendation and source identifiers', () => {
  ...
  })).toThrow('Duplicate recommendation id: rec-duplicate');
  expect(() => buildPlatformAnalysisImportPlan({
    topicId: TOPIC_ID,
    analysis: { recommendations: [recommendation] },
    sourceMappings: [mapping, mapping],
  })).toThrow('Duplicate source mapping: utterance-1');
});
...
test('rejects issue fields that cannot satisfy the platform schema', () => {
  ...
  expect(() => build({ stance: 'agree' })).toThrow('Invalid stance: agree');
  expect(() => build({ frequency_class: 'unanimous' })).toThrow('Invalid frequency class: unanimous');
...
test('rejects empty plans and duplicate candidate provenance', () => {
  ...
  expect(() => buildPlatformAnalysisImportPlan({
    ...
    sourceMappings: [mapping],
  })).toThrow('Duplicate cited source: utterance-1');
  ...
  expect(() => buildPlatformAnalysisImportPlan({
    ...
    sourceMappings: [mapping],
  })).toThrow('Duplicate candidate id: rec-duplicate-minority:same');
});
...
test('writes a dry-run plan through the CLI without database credentials or mutation', () => {
  ...
  try {
    ...
    expect(plan).toMatchObject({ dryRun: true, databaseMutationExecuted: false, requiresHumanReview: true });
...
test('deduplicates import links by item while preserving source UIDs and rejects cluster conflicts', () => {
  ...
  })).toThrow(`Conflicting cluster mappings for item: ${sharedItemId}`);
});
