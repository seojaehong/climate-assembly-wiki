...
- 로그인 진입·로그인 실패·인증 셸·접근성 성명·미공개/공개 결과·온톨로지 검수 7개 경로를 데스크톱·모바일로 나눠 HTTP 상태, axe 자동 규칙, 건너뛰기 링크 포커스, 콘텐츠 폭과 내부 표 키보드 스크롤을 검사한다. 로그인 진입 경로는 합성 값을 채운 뒤 이메일→비밀번호→로그인→접근성 링크의 순방향·역방향 실제 Tab 순서와 양 끝 포커스 탈출도 확인한다.
