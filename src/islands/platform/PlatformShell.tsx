...
export default function PlatformShell() {
  ...
  if (session === undefined) {
    return <Centered><p style={{ color: MUTED, fontSize: 16 }}>불러오는 중…</p></Centered>;
  }
  if (session === null) {
...
function LoginCard({ notice, onSignedIn }: { notice: string | null; onSignedIn: (s: AuthSessionInfo) => void }) {
  ...
  const field: React.CSSProperties = {
    ...
    border: `1px solid #C4D8E4`, borderRadius: 12, marginBottom: 12, outline: 'none', color: INK,
  };

  return (
    <Centered>
      <div style={{ width: '100%', maxWidth: 420, background: '#fff', border: `1px solid ${LINE}`, borderRadius: 24, padding: '40px 32px', boxShadow: '0 8px 24px -16px rgba(31,78,121,.18)' }}>
        ...
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="이메일" style={field} autoComplete="username" />
        <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submit()} placeholder="비밀번호" style={field} autoComplete="current-password" />

        {err ? <p style={{ color: '#B91C1C', fontSize: 14, fontWeight: 600, margin: '2px 0 12px' }}>{err}</p> : null}
        {notice && !err ? <p style={{ color: '#B5651D', fontSize: 13, margin: '2px 0 12px' }}>{notice}</p> : null}
...
function AppShell({ session, scope, navigate }: { session: AuthSessionInfo; scope: Scope; navigate: (s: Scope) => void }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, display: 'flex', flexDirection: 'column', fontFamily: "Pretendard, system-ui, sans-serif" }}>
      {/* 상단 바 + 브레드크럼(=데이터 경로) */}
      <header style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 20px', borderBottom: `1px solid ${LINE}`, background: PANEL }}>
        ...
        <nav aria-label="브레드크럼" style={{ flex: 1, minWidth: 0, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 4 }}>
          {crumbs.length === 0 ? (
            ...
            <span key={c.key} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              {i > 0 ? <span style={{ color: '#C4D8E4' }}>/</span> : null}
              <button
                onClick={() => navigate(scope.view ? { ...c.scope, view: scope.view } : c.scope)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: i === crumbs.length - 1 ? 800 : 600, color: i === crumbs.length - 1 ? NAVY : '#2E75B6', padding: '2px 4px' }}
              >
                {c.label}
              </button>
            </span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => signOut()} style={{ fontSize: 13, fontWeight: 600, color: NAVY, background: '#fff', border: `1px solid ${LINE}`, borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>로그아웃</button>
        </div>
      </header>

      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        {/* 좌: 데이터 트리 네비 */}
        <aside style={{ width: 288, borderRight: `1px solid ${LINE}`, background: '#fff', padding: '16px 10px', overflowY: 'auto' }}>
          ...
          {loadingTree ? (
            <p style={{ color: MUTED, fontSize: 14, padding: '8px' }}>트리 불러오는 중…</p>
          ) : flat.length === 0 ? (
            <div style={{ padding: 8 }}>
              <p style={{ color: '#B5651D', fontSize: 13, lineHeight: 1.6 }}>{treeNotice ?? '표시할 공론화가 없습니다.'}</p>
          ...
          {treeNotice && flat.length > 0 ? <p style={{ color: '#B5651D', fontSize: 12, padding: 8 }}>{treeNotice}</p> : null}
        </aside>
...
function ViewTabs({ scope, navigate }: { scope: Scope; navigate: (s: Scope) => void }) {
  ...
  delete base.view;
  return (
    <div style={{ display: 'flex', gap: 6, marginBottom: 20, borderBottom: `1px solid ${LINE}`, paddingBottom: 12, flexWrap: 'wrap' }}>
      <button
        onClick={() => navigate(base)}
        style={tabStyle(!scope.view)}
      >개요</button>
      {views.map((v) => (
        <button key={v} onClick={() => navigate({ ...base, view: v })} style={tabStyle(scope.view === v)}>
          {viewLabel(v)}
        </button>
      ))}
    </div>
...
function tabStyle(active: boolean): React.CSSProperties {
  return {
    ...
    border: active ? `1px solid ${TEAL}` : `1px solid ${LINE}`,
