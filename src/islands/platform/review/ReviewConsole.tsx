...
const btn = (kind: 'primary' | 'ghost' | 'danger' | 'disabled'): React.CSSProperties => {
  ...
  if (kind === 'danger') return { ...base, background: '#fff', color: RED, border: `1px solid #F1C4C4` };
  if (kind === 'disabled') return { ...base, background: '#CBD5DC', color: '#fff', cursor: 'default' };
  return { ...base, background: '#fff', color: NAVY, border: `1px solid ${LINE}` };
};

const field: React.CSSProperties = {
  width: '100%', boxSizing: 'border-box', height: 44, padding: '0 12px', fontSize: 14,
  border: `1px solid #C4D8E4`, borderRadius: 10, outline: 'none', color: INK, background: '#fff',
};
...
export default function ReviewConsole({ topicId, items: itemsOverride }: ReviewConsoleProps) {
  ...
  if (!topicId) {
    return (
      <div style={{ border: `1px dashed ${TEAL}`, borderRadius: 14, padding: '22px 20px', background: PANEL, color: MUTED, fontSize: 15 }}>
  ...
  return (
    <div style={{ fontFamily: 'Pretendard, system-ui, sans-serif', color: INK }}>
      {/* 헤더 + 코드 seam */}
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 14, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
          <div>
            <label style={{ display: 'block', fontSize: 12, color: MUTED, marginBottom: 4 }}>조 참여 코드(join_code)</label>
            <input
              value={code}
      ...
      <p style={{ fontSize: 12, color: AMBER, background: '#FBF3E6', border: `1px solid #F0DFC0`, borderRadius: 10, padding: '8px 12px', margin: '0 0 14px' }}>
        ※ 검수 RPC는 조 <code>join_code</code>(운영자) 서명입니다. staff Auth 셸에서도 코드 입력 자리가 필요합니다(Phase 2에서 HQ 토큰 전환 예정, 미결).
      </p>

      {notice ? (
        <p style={{ fontSize: 14, color: AMBER, background: '#FBF3E6', borderRadius: 10, padding: '10px 14px', margin: '0 0 14px' }}>{notice}</p>
      ) : null}
      {flash ? (
        <p style={{ fontSize: 14, fontWeight: 600, color: NAVY, background: '#E4F2F6', borderRadius: 10, padding: '10px 14px', margin: '0 0 14px' }}>{flash}</p>
      ) : null}

      {/* 공개 게이트 안내 */}
      {list ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, padding: '10px 14px', borderRadius: 12, background: canPublish(reviewedCount) ? '#E3F1E6' : PANEL, border: `1px solid ${canPublish(reviewedCount) ? '#BFE0C6' : LINE}` }}>
      ...
      {list ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) minmax(340px, 1.3fr)', gap: 18, alignItems: 'start' }}>
          ...
          <section>
            ...
            <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {issues.length === 0 ? (
                ...
                return (
                  <li key={vm.id}>
                    <button
                      onClick={() => selectIssue(vm)}
                      style={{
                        width: '100%', textAlign: 'left', cursor: 'pointer',
                        border: active ? `1.5px solid ${TEAL}` : `1px solid ${LINE}`,
                    ...
                    >
                      ...
                      <div style={{ fontSize: 15, fontWeight: 700, color: INK, lineHeight: 1.4 }}>{vm.label}</div>
            ...
            {/* 병합 */}
            <div style={{ marginTop: 18, border: `1px solid ${LINE}`, borderRadius: 14, padding: 14, background: '#fff' }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                <select value={mergeSrc} onChange={(e) => setMergeSrc(e.target.value)} style={{ ...field, height: 40, width: 'auto', flex: 1, minWidth: 120 }}>
                  <option value="">원본…</option>
                  {issues.map((i) => <option key={i.id} value={i.id}>{i.label}</option>)}
                </select>
                <span style={{ color: MUTED }}>→</span>
                <select value={mergeDst} onChange={(e) => setMergeDst(e.target.value)} style={{ ...field, height: 40, width: 'auto', flex: 1, minWidth: 120 }}>
          ...
          <section style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {/* 편집 */}
            <div style={{ border: `1px solid ${LINE}`, borderRadius: 16, padding: 18, background: '#fff' }}>
              ...
              <label style={{ display: 'block', fontSize: 12, color: MUTED, marginBottom: 4 }}>쟁점명(label)</label>
              <input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="예: 재생에너지 발전 비중 확대" style={{ ...field, marginBottom: 12 }} />

              <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 12, color: MUTED, marginBottom: 4 }}>빈도(합의도)</label>
                  <select value={form.frequency} onChange={(e) => setForm({ ...form, frequency: e.target.value })} style={{ ...field }}>
                    <option value="">미지정</option>
                    {FREQUENCY_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: 12, color: MUTED, marginBottom: 4 }}>방향(입장)</label>
                  <select value={form.stance} onChange={(e) => setForm({ ...form, stance: e.target.value })} style={{ ...field }}>
                    <option value="">미지정</option>
              ...
              <label style={{ display: 'block', fontSize: 12, color: MUTED, marginBottom: 4 }}>요약(summary)</label>
              <textarea value={form.summary} onChange={(e) => setForm({ ...form, summary: e.target.value })} rows={3} placeholder="쟁점 확정문(사람 검수)" style={{ ...field, height: 'auto', padding: 12, resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.5, marginBottom: 12 }} />
                ...
                {!itemsLoaded ? (
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', borderTop: `1px solid ${LINE}`, paddingTop: 12 }}>
                      <select value={reclassTarget} onChange={(e) => setReclassTarget(e.target.value)} style={{ ...field, height: 40, width: 'auto', flex: 1, minWidth: 140 }}>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <label style={{ fontSize: 12, color: MUTED }}>cluster_id(합의도 분모 보정, 이동 시 대상 원문에 일괄 적용)</label>
                      <input value={clusterId} onChange={(e) => setClusterId(e.target.value)} placeholder="선택 — 같은 원문 군집 uuid" style={{ ...field, marginTop: 4, fontFamily: 'monospace', fontSize: 13 }} />
            ...
            {/* 미분류함 */}
            <div style={{ border: `1px solid ${LINE}`, borderRadius: 16, padding: 18, background: PANEL }}>
              ...
              {!itemsLoaded ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {partition.unclassified.map((it) => (
                    <div key={it.itemId} style={{ border: `1px solid ${LINE}`, borderRadius: 12, padding: '10px 12px', background: '#fff' }}>
        ...
        <div style={{ border: `1px dashed ${TEAL}`, borderRadius: 14, padding: '22px 20px', background: PANEL, color: MUTED, fontSize: 14 }}>
...
function ItemCard({ it, checked, onToggle }: { it: ReviewItem; checked: boolean; onToggle: () => void }) {
  return (
    <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', border: `1px solid ${checked ? TEAL : LINE}`, borderRadius: 12, padding: '10px 12px', background: checked ? '#F1FAFB' : '#fff', cursor: 'pointer' }}>
