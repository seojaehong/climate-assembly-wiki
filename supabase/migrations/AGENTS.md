...
## 반환 컬럼을 늘리고 싶을 때 — 먼저 **작은 함수를 새로 두는 길**을 재라 (2026-09-02, s19)

본부 화면이 꼭지 마감을 되읽어야 했다. 눈에 먼저 들어오는 길은 `hq_submissions` 의 반환에
`deadline_at` 을 얹는 것이다. 그러려면 반환 타입이 바뀌므로 **drop 후 create** 여야 하는데,

- `hq_submissions` 는 본부 보드가 **5초마다 부르는 핫 함수**다(운영 중 스왑)
- `drop` 은 ACL 을 날린다 — 새 함수는 PUBLIC EXECUTE 를 기본으로 갖고 명시 grant 는 사라진다
  (위 「`drop function` 뒤 권한이 실제로 어떻게 되나」)

그래서 `20260902_s19_hq_topic_deadlines.sql` 은 **반환 3컬럼짜리 소형 RPC 를 하나 더 두는**
쪽을 골랐다. `create or replace` 로 충분하고(ACL 보존) 기존 함수·표·권한을 하나도 안 건드린다.

★ 판단 기준 — **읽는 쪽이 그 컬럼을 꼭 같은 호출에서 받아야 하는가.** 아니라면 함수를 하나
더 두는 편이 거의 언제나 싸다. 호출이 하나 늘어나는 비용 < 핫 함수 스왑 + ACL 재부여 위험.

★ 그리고 **새 RPC 는 미적용 상태에서 화면이 죽지 않게 짜라.** PostgREST 는 함수가 없으면
`PGRST202` 를 낸다. 래퍼가 그 코드만 골라 「모름」(null)으로 돌려주면 마이그레이션 적용과
배포 순서가 서로에게 묶이지 않는다(s17 의 선택 필드 `?:` 와 같은 규칙, `src/lib/hq-submissions.ts`).
