...
- **A2 preflight 제거 후 읽기 전용 검증** (`activation_preflight_post_remove.sql`): rollback 뒤 함수가 남아 있지 않은지 확인하고 `database_mutation_executed=false` 결과만 반환한다. P1C schema rollback 전에 실행한다.
...
# A2 preflight rollback 직후 = psql ... -f /tmp/activation_preflight_post_remove.sql
```
