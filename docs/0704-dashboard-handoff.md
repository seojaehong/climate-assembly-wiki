...
## Supabase 운영규정 투표 레퍼런스

The previous regulation-vote reference is still useful as a result-screen pattern, even though the 7.4 conditional votes currently use Google Forms for field reliability.

- Live-style result board: `/regulation-vote/`
- Final archived summary board: `/archive/2026-06-14/regulation_R1-R9_results.html`
- Source note in archive: `Supabase climate_vote.votes`
- Reusable UX pattern: response count in the header, option bars per vote item, winner/decision highlight, and explicit lost/skipped states.
- Important caveat from the archive: one round notes data loss and missing PITR, so 7.4 uses Google Form + Sheet + exported JSON unless a Supabase schema/permissions change is explicitly approved.

The 0704 conditional-vote console copies the usable presentation pattern without opening a new Supabase path:

- QR is shown through a large projector modal from the `QR 탭` button.
- `결과보기` reads `/0704-admin/decision-votes-report.json` and shows response count plus option bars.
- Results are marked as admin/capture use only until voting is closed.
- Claude or another operator can capture the result modal after `scripts\refresh-0704-decision-votes.ps1` updates the JSON.
