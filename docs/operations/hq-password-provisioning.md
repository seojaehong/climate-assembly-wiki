...
- 2026-07-26: `hq_password` 최초 설정 완료(bcrypt `$2a$`). 라이브에서 오답 1회 호출로
  `attendance_hq_unlock`이 `null`을 반환하는 것까지 확인.
- 이 회차는 운영 판단으로 12자 기준을 적용하지 않았다. **기준 자체는 아래 그대로 유지한다** —
  다음 행사에서 기본값이 낮아지지 않게 하기 위함이다.
- 행사 종료 후 교체를 권장한다(교체 절차는 아래).
