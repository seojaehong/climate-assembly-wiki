...
- 새 명부는 숙의 182개 행 중 명시적 미참석 26개를 제외한 **156명**을 운영 출석부 기준으로 사용한다. 이전 참석현황과 공식 ID를 비공개 대조한 결과, 조 제목보다 많은 6명 중 5명은 이전에도 참석이었고 1명은 불참에서 참석으로 변경됐으므로 조 제목 합계 150명은 본문 갱신 누락으로 판정했다. 계획안의 147명은 계획 인원으로 보존하되 운영 DB 적재는 최신 명부 본문 156명을 사용하며, 실제 적재는 사용자 승인 뒤에만 수행한다.
...
- [x] 추진계획안의 계획 인원 147명과 최종 명부의 운영 출석 156명을 구분하고, 이전 명부와 비공개 행 대조로 조 제목 150명의 갱신 누락을 확인했다. 운영 명부는 최종 HWPX SHA-256 `549E9BB9770F05CBD1B99B6C13A1D5F959E24FC7BE612D699A4F7FF5C5758C4B`의 참석 행 156명·15개 조를 기준으로 한다.
...
|---|---|---|
| 1. 명단 확정·적재 패킷 생성 | `python scripts/import-attendance-roster.py --file "<최종 HWPX>" --session-slug 0912-deliberation --attendance-only --expected-count 156 --sql-output "<승인된 비공개 경로>"` | 최종 HWPX 해시, 182개 원행, 명시적 미참석 26명, 선택 156명, 15개 조, 빈 값·중복 0건을 fail-closed로 확인한다. SQL은 개인정보를 포함하므로 저장소·일반 로그·Notion에 두지 않는다. 현재 승인 후보 SQL은 같은 importer로 재생성한 내용과 일치하며 적용은 별도 사용자 승인을 기다린다. |
| 2. P1 tenancy — 미적용 시 별도 운영 승인 | `supabase/migrations/platform_p1_tenancy.sql` | migration 이력과 정본 checksum을 확인한다. 이미 적용됐으면 건너뛰고, 미적용이면 사용자 승인 뒤 먼저 적용한다. seed와 s22는 `org_id`·`assembly_id`를 쓰므로 P1보다 앞서 실행하면 안 된다. |
...
| 8a. 운영 역할·HQ 보정·명부 적용 — **별도 데이터 변경 승인** | 승인된 비공개 membership SQL → 비공개 HQ credential 보정 SQL → 1번에서 생성한 156명 명부 SQL | 사용자 Auth 계정에 정확히 `org_admin`, `hq` 두 활성 역할을 연결하고 count-only `platform_activation_preflight()`가 `ready`인지 확인한다. 초기 비밀번호 정책 불일치 1건만 비공개 편집기에서 보정한 뒤 8개 운영자 자격정보가 모두 정책과 일치하는지 건수만 재확인한다. 마지막으로 156명 명부를 적용해 active assignment·attendance 각 156건과 최종 source hash를 검증한다. 비밀번호·성명·공식 ID는 일반 로그에 남기지 않는다. |
...
순서가 어긋났거나 기대 건수가 다르면 즉시 중단한다. 검증된 핵심 migration 순서는 `P1 → seed/s22 → P1a → P2 → P1b/P1c → 역할·HQ 보정·156명 명부 → P2a → P3 → P4`다. 여기서 `s22`가 9월 10일 정본의 8단계 교정본이며 s20·s21은 계속 동결한다. 현장 절차는 `최종 HWPX 156명 검증·비공개 SQL 생성 → P1 적용 이력·checksum 확인 및 미적용 시 승인·적용 → atomic seed/sync SQL 별도 승인·적용 → s22 별도 승인·적용 → P1a 승인·검증 → 4인자 HQ rotate 선교체 → P2 및 P1b/P1c 승인·검증 → 역할·HQ 보정·156명 명부 별도 승인·적용 → activation preflight ready 확인 → maintenance token/staff client 배포 → P2a 별도 승인·원자 cutover → positive/legacy negative 검증 → P3 별도 승인·검증 → P4 별도 승인·검증 → post-P4 legacy negative 재검증 → 최종 상태` 순서를 바꾸지 않는다. 앞선 승인은 뒤 단계 승인을 포함하지 않으며, 사용자의 명시적 운영 승인 전에 어느 DB 단계도 적용하거나 코드를 교체하지 않는다.

구형 검증 기록에 남은 `seed/s20` 또는 `seed/s21` 순서는 과거 교정 파일을 식별하는 동결 표식일 뿐 실행 순서가 아니다. 운영 실행은 바로 위의 `seed/s22` 순서만 따른다.
