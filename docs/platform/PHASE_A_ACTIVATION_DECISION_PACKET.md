...
| A4 | 계약 `fa4d0fe`, plan `4ab55b5`, migration 계열 `873a50f..e9721eb`, approval/receipt·injected execution/reconciliation lifecycle core, 휴면 read-only ledger status RPC, 로컬 durable crash/restart·revocation/context·비식별 전체-store/keyed receipt·off-store inventory checkpoint·별도 local anchor persistence rehearsal, key lifecycle registry 정책·revision-bound live authorization contract와 injected lifecycle wrapper, CI | 초안·격리 PostgreSQL·response-loss/reconciliation·재시작/CAS·취소/claim 경쟁·현재 entry 무결성·합성 단일-key HMAC·active/verify-only/retired cutoff·membership/org ABA 차단·registry-backed 실행/만료 후 reconciliation·checkpoint 이후 삭제/tail 변경·anchor append 응답 유실/충돌/변조 검증 | production migration, 실제 mapping/readiness, RPC 권한, authoritative revocation·운영 외부 durable anchor/독립 freshness·production key custody/회전 adapter·production-grade durable approval/receipt·live membership row-version transaction adapter·executor/status adapter |
...
- 현재 blocker: 외부 HMAC approval artifact, role·expiry·revocation·one-time claim·terminal finalization, 비식별 receipt, injected response-loss lifecycle과 기존 claim 전용 명시적 reconciliation core, 휴면 read-only ledger status RPC 초안, 저장소 밖 로컬 durable crash/restart·revocation/context와 현재 entry 전체-store/keyed receipt audit, off-store inventory checkpoint와 별도 local anchor persistence rehearsal, active/verify-only/retired key registry 정책, revision-bound membership/org ABA 차단 계약과 registry-composed lifecycle wrapper는 구현됐고 receipt 없는 기존 claim의 RPC 자동 재시도도 차단했다. 그러나 실제 key custody·회전 adapter와 운영 절차, authoritative revocation·운영 외부 durable anchor·독립 freshness authority, production-grade durable approval/append-only receipt state·live membership row-version transaction adapter와 production executor·status adapter·권한은 없다. A4 plan은 계속 `readyForExecution:false`이며 이 production 경로와 아래 선행조건을 별도 repository 변경·사용자 승인으로 완성하기 전에는 F-A4를 승인 요청하지 않는다.
- RPC/ledger 호출에는 이제 검증한 approval·execution ID와 authorization revision을 exact fence로 전달하고 같은 revision 응답을 요구하는 capability 계약이 있다. 이 계약은 production executor/status adapter가 실제 Supabase transaction에서 live membership row version을 확인한다는 증거가 아니므로 기존 blocker와 승인 경계는 그대로다.
- Fence 왕복에 성공한 receipt는 authorization revision을 HMAC 결속하고 production-bound 복구에서 현재 claim과 exact 일치를 요구한다. 이는 unfenced legacy receipt replay를 차단하지만 production durable receipt 저장소나 live row-version adapter의 존재를 증명하지 않으므로 blocker는 해소되지 않는다.
