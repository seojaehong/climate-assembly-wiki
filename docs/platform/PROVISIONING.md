...
- 실행 승인 artifact의 adapter-independent 계약은 exact plan/source와 대상 organization UUID·host, canonical `org_admin|hq` reviewer, 최대 15분 유효기간과 외부 HMAC key를 결속한다. 검증 시 실행 context의 organization/host가 artifact와 일치하고 durable approval-state lookup이 있어야 한다. `revokedAt`이 있거나 다른 execution·organization·host가 이미 claim했거나 claim이 `completed|failed`이면 거부한다. claim이 없을 때의 `claim_required`는 실행 허가가 아니라 실제 adapter가 mutation 전에 원자적으로 claim해야 한다는 뜻이다. 동일 approval/execution/organization/host/plan의 진행 중 claim만 응답 유실 복구를 위해 재개할 수 있다. 현재 승인 발급 CLI, live membership 검증, durable revocation/claim 저장소와 adapter가 없으므로 plan의 blocker와 `readyForExecution:false`는 바뀌지 않는다.
