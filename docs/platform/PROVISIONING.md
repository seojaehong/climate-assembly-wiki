...
- 실행 승인 artifact의 adapter-independent 계약은 exact plan/source와 대상 organization UUID·host, 같은 canonical Auth 승인자/실행자와 `org_admin|hq` 역할, 최대 15분 유효기간과 외부 HMAC key를 결속한다. authorization adapter는 state와 live membership/org/host를 한 snapshot으로 읽고 같은 값을 expected snapshot으로 전달한 CAS claim 안에서 재검증해야 한다. core는 claim 응답의 transaction 시점 context를 다시 확인한다. `revokedAt`, 다른 execution·organization·host·actor·role, `completed|failed` claim을 거부하고 동일 진행 중 claim만 응답 유실 복구에 사용한다. in-memory adapter는 동시 claim에서 하나만 `new`가 되고 동일 claim 충돌은 `reconciled`되는 계약을 검증한다. 현재 승인 발급 CLI, 실제 key, durable 저장소와 production adapter가 없으므로 결과와 plan은 계속 `readyForExecution:false`, `rpcMutationExecuted:false`, `databaseMutationExecuted:false`다.
