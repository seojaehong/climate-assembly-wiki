...
- 추적 `evaluation/platform-a4-migration-bundle.json`은 plan core·서버 계약·SQL뿐 아니라 bundle builder와 A4 plan/bundle 집중 테스트까지 14개 artifact의 exact bytes를 결속한다. bundle 테스트는 이 추적 manifest를 매 실행 current source에서 재구성해 stale source hash와 자체 재봉인 drift를 거부한다. manifest 갱신은 production 승인이나 실행이 아니라 review candidate source refresh다.
