...
- execution receipt 순수 seal/verifier는 승인된 review plan의 operation/source를 유지한 실행형 RPC checksum을 결정적으로 파생하고 approved/executed checksum을 함께 결속한다. 성공 RPC 응답의 exact operation 순서·ID·resource UUID·team join code 형식을 검증한 뒤 resource ID와 join code를 제거하고 비식별 operation 상태·count만 HMAC으로 봉인한다. 실패는 allowlist `design_*` 코드와 `rollbackVerified:true`가 함께 있어야 하며 미확정 response-loss를 실패로 확정하지 않는다. injected lifecycle core는 receipt를 claim 전·직후 조회하고 `claim→RPC→append-only receipt→재조회→finalize`를 조율한다. append 응답 유실 뒤에는 저장된 HMAC receipt를 검증해 RPC를 재호출하지 않고 finalize하며, 저장 성공 응답 뒤에도 receipt가 재조회되지 않거나 달라지면 claim을 열어 둔다. in-memory receipt adapter만 제공하므로 production persistence·RPC 연결은 없다.
