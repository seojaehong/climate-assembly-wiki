...
- execution receipt 순수 seal/verifier는 승인된 review plan의 operation/source를 유지한 실행형 RPC checksum을 결정적으로 파생하고 approved/executed checksum을 함께 결속한다. 성공 RPC 응답의 exact operation 순서·ID·resource UUID·team join code 형식을 검증한 뒤 resource ID와 join code를 제거하고 비식별 operation 상태·count만 HMAC으로 봉인한다. 실패는 allowlist `design_*` 코드와 `rollbackVerified:true`가 함께 있어야 하며 미확정 response-loss를 실패로 확정하지 않는다. 이 함수들은 RPC나 persistence를 호출하지 않는다. production 순서는 claim→RPC→append-only receipt 저장→finalize이며 저장 실패 시 claim을 열어 둔다.
