...
`platform-design-provisioning-durable-store.mjs`는 production 연결 전 crash/restart 계약을 검증하는 로컬 rehearsal 전용 adapter다. 저장소 밖의 marker가 있는 빈 디렉터리에서 approval별 immutable hash-chain journal과 execution별 append-only receipt를 사용하고, CAS 경쟁·기존 receipt 복구·receipt 충돌을 재현한다. unclaimed approval의 local revocation과 synthetic membership context 무효화도 같은 journal CAS로 기록해 claim 경쟁과 finalize 차단을 검증한다. 다음 sequence record의 hard-link 게시가 lock-free CAS가 되므로 persistent lock은 만들지 않고, crash가 남긴 규격화된 temp는 무시한다. fixture context는 identity를 유지한 `active→inactive`만 허용하며 live membership 또는 재활성화 모델이 아니다. SHA-256 journal chain도 외부 신뢰 서명이 아니고 production credential·Supabase client·RPC executor를 읽거나 호출하지 않는다.
