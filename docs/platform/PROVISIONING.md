...
Execution adapter는 append-only receipt persistence를 명시하고 `runId`별 read와 append를 모두 제공해야 한다. Core는 operation lookup 전에 저장된 receipt를 읽어 exact HMAC·plan·approval·run을 검증하고 일치하면 mutation 없이 그대로 반환한다. 이미 봉인된 terminal receipt 복구는 원 approval의 서명·발급시각 결속을 다시 확인하므로 15분 실행 창이 지난 재시작에서도 mutation 없이 가능하지만, 저장본이 없으면 현재 trusted clock 기준 approval freshness를 다시 통과해야 신규 실행할 수 있다. 신규 실행은 receipt append 응답을 성공 근거로 신뢰하지 않고 같은 `runId`를 다시 읽어 생성한 receipt와 canonical byte 의미가 같을 때만 완료한다. append 뒤 응답 유실은 저장본 재조회로 복구하지만 누락·위조·다른 run/approval·충돌 receipt는 실패하며 operation을 자동 재실행하지 않는다. Adapter가 반환하거나 받는 객체는 검증 경계에서 복제해 사후 변조가 core의 검증본을 바꾸지 못하게 한다.

`platform-access-provisioning-durable-store.mjs`는 이 read/append 계약을 process restart에서 검증하는 로컬 rehearsal 전용 receipt adapter다. 저장소 밖의 기존 빈 절대경로에 exact marker와 `receipts/`를 만들고, HMAC·plan·key·run을 다시 검증한 비식별 receipt만 임시파일→hard-link 원자 게시로 append한다. 같은 run의 exact receipt는 `existing`, 다른 유효 receipt는 원본을 덮지 않고 `conflict`이며 core는 read-back 불일치로 실행 성공을 거부한다. marker·receipt directory·receipt file의 symlink/junction 탈출, repository 내부·상대·비어 있지 않은 초기 경로, 위조·과대 receipt를 fail-closed한다. 이 store는 production invitation ledger·메일 provider·권한 상태·외부 anchor·key custody를 제공하지 않고 Supabase·Auth·credential을 읽지 않는다.
...
다만 production Supabase/Auth adapter와 CLI 실행 경로는 아직 연결하지 않았다. 특히 현재 `invitation` table에는 `(org_id,email,role)` unique 또는 별도 operation ledger가 없어 응답 유실 뒤 중복 초대·메일을 서버가 확실히 막을 수 없다. 이 멱등 저장 계약, 초대 메일 provider, production-grade 외부 append-only receipt 저장소를 별도 승인·검증하기 전에는 executor core에 production adapter를 주입하거나 계획을 수동 SQL/API 작업 목록으로 사용하지 않는다.
...
로컬 anchor 리허설은 `initializeLocalDesignProvisioningCheckpointAnchorStore()`로 rehearsal store와 repository 밖의 별도 빈 절대경로를 초기화하고 `createLocalDesignProvisioningCheckpointAnchorAdapter()`를 만든 뒤 `persistLocalDesignProvisioningRehearsalStoreCheckpoint()`에 주입한다. Checkpoint는 생성시각 해시별 불변 hard-link record로 게시되고 즉시 authoritative read-back된다. 동일 bytes 재게시는 멱등이며 저장 후 응답 유실은 재조회로 복구하지만 같은 생성시각의 다른 bytes, marker/layout drift, record 변조와 symlink/junction은 거부한다. 이는 같은 호스트 로컬 파일시스템 계약이며 운영 외부 durable anchor·독립 timestamp·production key custody를 대신하지 않는다.
...
`platform-design-provisioning-durable-store.mjs`는 production 연결 전 crash/restart 계약을 검증하는 로컬 rehearsal 전용 adapter다. 저장소 밖의 marker가 있는 빈 디렉터리에서 approval별 immutable hash-chain journal과 execution별 append-only receipt를 사용하고, CAS 경쟁·기존 receipt 복구·receipt 충돌을 재현한다. marker와 record boundary는 `immutable_hard_link_v1` CAS 방식을 고정한다. unclaimed approval의 local revocation과 synthetic membership context 무효화도 같은 journal CAS로 기록해 claim 경쟁과 finalize 차단을 검증한다. 다음 sequence record의 hard-link 게시가 lock-free CAS가 되므로 persistent lock은 만들지 않는다. crash temp는 존재하면 owned regular file인지 검사하지만 다른 publisher가 열거 직후 정상 unlink해 사라진 경우는 경쟁 오류로 보지 않는다. 독립 Node 프로세스 6개의 같은-snapshot claim 경쟁도 하나만 게시되는지 실행 검증한다. read-only 전체-store audit는 존재하는 모든 journal·receipt와 현재 claim 연결을 비식별 건수로 검사하고 예상하지 않은 root/receipt entry를 거부한다. 기본은 keyless라 서명 미검증이며, 합성 HMAC key와 exact key ID를 호출 인자로 함께 주입한 rehearsal만 모든 현재 receipt digest를 상수시간 비교한다. immutable catalog가 없어 삭제 entry 완전성은 검증하지 못하고 단일 로컬 key도 production key custody·회전을 대신하지 않는다. fixture context는 identity를 유지한 `active→inactive`만 허용하며 live membership 또는 재활성화 모델이 아니다. SHA-256 journal chain도 외부 신뢰 서명이 아니고 production credential·Supabase client·RPC executor를 읽거나 호출하지 않는다.
...
- 2026-08-25 migration 초안 작성은 승인되어 session 계약 재확인, `(session_id, ordinal)` team identity, dormant design RPC·멱등 ledger·transaction 내부 join code 생성과 read-only reconciliation status RPC 초안, rollback·검증 묶음을 작성했다. status RPC는 비식별 checksum·operation query만 받고 현재 기관 `org_admin|hq`를 재검증하며, ledger 일부 누락은 `pending`, checksum/type/resource 불일치는 충돌로 처리한다. `STABLE` 함수이고 모든 기본 역할의 EXECUTE가 회수돼 있다. 현재 blocker는 production 적용 미승인, migration 미적용, RPC·ledger·join code 경로 미활성화다. `readyForExecution:false`인 plan을 SQL/API 작업 목록으로 사용하거나 production executor에 연결하지 않는다.
- 실행 승인 artifact의 adapter-independent 계약은 exact plan/source와 대상 organization UUID·host, 같은 canonical Auth 승인자/실행자와 `org_admin|hq` 역할, 최대 15분 유효기간과 외부 HMAC key를 결속한다. authorization adapter는 state와 live membership/org/host를 한 snapshot으로 읽고 같은 값을 expected snapshot으로 전달한 CAS claim·finalize 안에서 재검증해야 한다. core는 두 CAS 응답의 transaction 시점 context를 다시 확인한다. `revokedAt`, 다른 execution·organization·host·actor·role을 거부하고 동일 진행 중 claim과 동일 terminal outcome만 응답 유실 복구에 사용한다. 유효 시간 안에 시작된 claim은 만료 뒤에도 감사 상태를 `completed|failed`로 닫을 수 있지만 반대 outcome 덮어쓰기와 terminal state 재-claim은 금지한다. in-memory adapter는 동시 claim·finalize에서 각각 하나만 `new`가 되고 동일 경쟁은 `reconciled`되는 계약을 검증한다. 현재 승인 발급 CLI, 실제 key, durable 저장소와 production adapter가 없으므로 결과와 plan은 계속 `readyForExecution:false`, `rpcMutationExecuted:false`, `databaseMutationExecuted:false`다.
- execution receipt 순수 seal/verifier는 승인된 review plan의 operation/source를 유지한 실행형 RPC checksum을 결정적으로 파생하고 approved/executed checksum을 함께 결속한다. 성공 RPC 응답의 exact operation 순서·ID·resource UUID·team join code 형식을 검증한 뒤 resource ID와 join code를 제거하고 비식별 operation 상태·count만 HMAC으로 봉인한다. 실패는 allowlist `design_*` 코드와 `rollbackVerified:true`가 함께 있어야 하며 미확정 response-loss를 실패로 확정하지 않는다. injected lifecycle core는 receipt를 claim 전·직후 조회하고 `claim→RPC→append-only receipt→재조회→finalize`를 조율한다. `new` claim 소유자만 RPC를 호출하고, receipt 없는 기존·reconciled claim은 자동 재시도 없이 명시적 reconciliation을 요구한다. 별도 reconciliation lifecycle은 기존 active claim에서만 비식별 checksum·operation lookup query를 adapter에 전달하며 실행 plan·원본 bytes·새 claim·execution 호출을 만들지 않는다. migration 초안의 read-only status RPC가 같은 query로 현재 기관 ledger/resource를 조회해 `pending|completed`를 반환하지만 모든 실행 권한은 회수돼 있다. `pending`과 lookup 오류는 claim을 열어 두고, 완료 결과만 exact 검증·redaction·receipt persistence 뒤 종결한다. append 응답 유실 뒤에는 저장된 HMAC receipt를 검증해 RPC를 재호출하지 않고 finalize하며, 저장 성공 응답 뒤에도 receipt가 재조회되지 않거나 달라지면 claim을 열어 둔다. production persistence·권한·adapter 연결은 없다.
- 추적 `evaluation/platform-a4-migration-bundle.json`은 plan core·서버 계약·SQL뿐 아니라 bundle builder, durable store, key registry 정책 wrapper, A4 plan/bundle 집중 테스트, CI workflow와 workflow LF 규칙까지 18개 artifact의 Git-canonical bytes를 결속한다. bundle 테스트는 이 추적 manifest를 매 실행 current source에서 재구성해 stale source hash와 자체 재봉인 drift를 거부한다. manifest 갱신은 production 승인이나 실행이 아니라 review candidate source refresh다.
- schema v2 preflight의 `readyForAdditiveMigration`은 기존 team을 보존한 채 nullable ordinal과 제약 초안을 설치할 수 있는지만 뜻한다. `readyForActivation`은 별도 승인된 기존 행 mapping 뒤 session/team 필수값·중복·부모/org 불일치가 모두 0일 때만 true다. 두 상태 모두 별도 production 승인 없이는 실행 권한이 아니다.
- rollback은 ledger 또는 non-null team ordinal이 존재하면 `design_provisioning_rollback_requires_data_plan`으로 원자적으로 거부한다. 실제 사용 뒤에는 감사기록·생성 resource·mapping 보존 범위를 정한 별도 승인 rollback 계획이 필요하다.
