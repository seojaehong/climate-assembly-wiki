...
- A4는 UI와 공용 contract를 쓰는 저장소 외부 dry-run provisioning plan까지만 허용한다. assembly→session→topic/team stable operation과 exact source 재생성 검증은 준비하되, 회차 title/slug 저장·team join code·멱등 ledger·design RPC의 migration 계약이 승인되기 전에는 항상 `readyForExecution:false`이며 executor를 연결하지 않는다.
