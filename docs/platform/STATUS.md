...
**M3/M4 로컬 검수 작업대 검증(2026-08-12):** 공용 운영 내비게이션에 `/ko/moderator/ontology-review/`를 추가하고 sealed Canvas review plan과 정확한 원 snapshot을 브라우저 메모리에서만 대조하는 사람 검수 화면을 연결했다. 실제 Chromium은 plan·snapshot 업로드, 비식별 역할 ID 입력, node 수정 승인과 node·relation·cluster 반려 plan 다운로드, 같은 node ID를 가진 새 plan 재로딩 때 이전 입력 제거, 두 번째 plan의 node 2건·relation 2건·cluster 1건 승인 및 다운로드를 완료했다. 결과의 canonical 검수 시각·snapshot 43 출처·원문·선택 역할·관계 유형·대표 Issue·검수자와 `databaseMutationExecuted:false`, `publicGraphWritten:false`를 확인했다. 같은 실행에서 DB 쓰기 요청과 브라우저 오류는 0건이었고 graph·guide backing asset도 각각 2xx였다. 전체 src·scripts Vitest 60개 파일·879건, automation Vitest 18개 파일·205건, Astro check 오류 0건(기존 hint 49건), Node 20 정적 빌드 7,914페이지가 통과했다. 이 화면과 `noindex`는 접근통제가 아니며 reviewer 인증, DB queue/RLS/RPC, migration, 공개 graph 반영은 여전히 미구현·승인 대상이다.
