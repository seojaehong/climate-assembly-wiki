...
- 이 감사는 현재 존재하는 entry만 검증한다. 별도 외부 anchor가 없어 삭제된 approval/receipt의 완전성을 증명하지 못하므로 `catalogCompletenessVerified:false`를 유지한다. 로컬 단일-key 검증은 production key custody·회전 registry를 대신하지 않는다.
