...
- checkpoint는 생성 시점의 exact inventory와 state/receipt에 남은 canonical 사건시각, 주입된 로컬 검증시각 간 계약만 증명한다. timestamp가 없는 초기 authorization 생성과 synthetic context 전이의 실제 발생시각, 외부 보관, production key custody·회전 registry 또는 독립 timestamp authority를 구현하지 않으며 복원된 동일 bytes와 생성 전 손상을 구분하지 못한다.
