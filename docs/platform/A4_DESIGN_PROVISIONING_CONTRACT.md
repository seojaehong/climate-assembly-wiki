...
- 이 감사는 현재 존재하는 entry만 검증한다. 별도 immutable catalog가 없어 삭제된 approval/receipt의 완전성을 증명하지 못하고 외부 HMAC key도 받지 않아 receipt 서명 진위를 검증하지 않으므로 각각 `catalogCompletenessVerified:false`, `receiptSignatureVerified:false`를 명시한다.
