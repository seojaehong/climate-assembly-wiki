...
v4.11.0 이 걷어낸 HWP3 유실의 **잔여 전량**을 처리했습니다. rhwp `samples/` 의 HWP3 원본 9종을 한컴 변환본과 문자 멀티셋으로 대조하면, 이제 변환본에만 있는 문자는 그림 대체텍스트를 빼면 **0** 입니다.

- **🖼️ 그리기 개체 안 글상자 텍스트 회수 (#73)**: HWP3 의 ch=11(그림) 확장 블록은 `pic_type` 이 3 이면 그림이 아니라 **그리기 개체 트리**입니다. 표지 제목·순서도 라벨처럼 눈에 잘 띄는 본문이 그 안 글상자에 들어가는데, 종전에는 블록을 통째로 건너뛰어 사라졌습니다(sample16 618자, sample19 86자). 프레임 헤더 → 도형 트리(형제·자식 비트)를 훑어 글상자 문단 리스트를 본문과 같은 파서로 읽습니다. 비-글상자 도형이 '글상자로 만들기' 로 같은 블록을 실은 경우도, **잉여 길이가 스펙 표 78 과 정확히 맞을 때만** 회수합니다. 트리 해석은 확장 블록 슬라이스 안에서만 하므로 실패해도 본문 스트림 동기가 깨지지 않고 종전 동작으로 되돌아갑니다(재귀 256단계·도형 4,096개 상한).
- **🔣 비가시 마커가 대체 문자로 새던 것**: 새 번호·쪽 번호 위치·쪽 감추기 제어 문자 자리에 `￼`(U+FFFC)를 심어 마크다운에 그대로 남았습니다. 한글도 그 자리에 아무 글자를 내지 않습니다 — 차례 표식과 같은 계약으로 맞췄습니다.

## v4.11.0 변경사항

rhwp 최신 devel(v0.8.3~0.8.4)을 훑어 kordoc 의 HWP3 디코더에 반영했습니다. 네 결함 모두 **글자가 경고 없이 사라지는** 형태라, 변환은 성공했는데 본문 일부가 비어 있는 식으로만 드러납니다.

- **🈳 HWP3 기호·한자 유실 (완성형 좌표 규칙 누락)**: HWP3 은 KS X 1001 기호행(0xA1~0xAC)을 행 간격 96, 한자행(0xCA~0xFD)을 행 간격 94 로 편 코드에 문자를 담습니다. rhwp 에서 johab 디코더를 포팅할 때 하드코딩 표만 옮기고 **이 규칙 두 개를 빠뜨려**, 표에 없는 코드는 전부 삭제됐습니다 — `채권(債權)조서` 가 `채권()조서` 가 되고 `※ ○ 【】 ∴ ☞ ￦ ━` 같은 기호가 통째로 사라집니다. 규칙을 이식하고, 표준 매핑과 한컴 표기가 갈리는 두 자리(`∼→～`, `⊙→◉`)를 보정했습니다.
- **🔤 사적 코드 실측 표 확장**: 라틴 확장(`ü·ö·ä·ß` 가 지워져 "für"→"fr" 이 되던 것)과 rhwp #5860 의 실측 28코드를 이식하고, rhwp 표에도 없던 코드를 직접 실측해 더했습니다 — 괘선 조각 6종, 겹줄, 원문자·괄호문자 계열(`⓪①②`·`ⓐ~ⓕ`·`㉠~㉢`), 글머리표 `•` 등. 근거는 HWP3 원본과 같은 문서의 한컴 변환본을 문맥 정렬한 것으로, **코드 출현 횟수가 변환본의 해당 문자 개수와 정확히 맞는 것만** 실었습니다.
- **✒️ 초성 없는 아래아 음절 유실**: 초성이 '채움'인 아래아 음절(`ㆍ` 단독)을 무효 초성으로 보고 버렸습니다. 한컴 변환본과 같이 자모 하나로 남깁니다.
- **🧱 서식 run 이 PUA 정리를 건너뛰던 결함 (전 포맷)**: 굵게·밑줄이 걸린 문단은 span 경로를 타는데 여기서만 PUA 치환·제거를 하지 않아, 한컴 전용 글머리표·괘선 조각이 **원시 PUA(두부 문자) 그대로** 마크다운에 실렸습니다. HWP5·HWPX·HWP3 공통 경로입니다. 함께 rhwp 의 한컴 PUA 검증표(한컴 PDF 대조 확정)에서 빠져 있던 22종(`┌┬┐└┘│`·`┈═━`·`⓪①②④~⑨`·`✺◇▸□`·`⊞⊟`)을 동기화했습니다.

검증은 rhwp `samples/` 의 HWP3 원본 9종을 같은 문서의 한컴 변환본과 문자 멀티셋으로 대조했습니다. 변환본에만 있는 문자(=HWP3 경로에서 잃은 문자)가 `hwp3-sample11` 189→0, `hwp3-sample16` 1,139→618, `hwp3-sample10` 172→11 로 줄었습니다. 남은 618자는 **그리기 개체 안 글상자**(표지 제목·순서도 라벨)로, kordoc 은 아직 그리기 개체를 건너뜁니다.
...
- **📌 다줄 출처행 왼쪽정렬 폴백**: `<right>` 문단(출처행·발신일자)이 본문폭을 넘겨 줄바꿈되면 오른쪽정렬 특성상 **왼쪽 끝이 줄마다 들쭉날쭉**해져 들여쓰기가 깨진 듯 보였습니다(한글 실렌더 QA — 3줄 출처행). 이제 조판 폭을 미리 계산해 한 줄에 들어가면 종전대로 우측 배치(실측 GT6/GT7/GT11 관행), 넘치면 왼쪽정렬로 방출합니다.
- **🔤 출처행 캡션체**: `출처:`·`자료:`로 시작하는 `<right>` 행은 본문과 같은 크기라 캡션으로 읽히지 않았습니다. 전용 charPr을 신설해 **본문보다 3pt 작게**(하한 10pt), 실측 프리셋(개조식·보고서·계획서)은 한양중고딕, 그 외 프리셋은 본문 폰트로 방출합니다. 표 아래 출처를 본문과 구분하는 실무 관행을 따른 것으로, 발신일자 등 다른 `<right>` 행은 본문 크기를 유지합니다.

## v4.9.1 변경사항

실제 자치구 검토보고서를 뽑아 한컴으로 열어보고, 눈에 걸리는 것을 코퍼스 555건 실측과 대조해 고쳤습니다. 세 결함 모두 **양식 파일 하나의 저장값을 실무 관행보다 앞세운 것**이 원인이었습니다.

- **📐 `○` 항목 사이 간격 정상화**: 문단 위 간격 기본값이 `□15pt/○10pt/-6pt`(「2_보고서 양식」 한 파일의 저장값)였는데, 코퍼스 실측은 정반대입니다 — `□` 582문단 중 **88%**, `○` 799문단 중 **88%**가 간격 **0**이고 종전 값은 상위 6위에도 없습니다(2위가 1.5pt). 줄간격 160%에 10pt가 더해져 항목 사이가 1.4줄로 벌어지던 것을 실측 최빈값으로 되돌렸습니다(`□`만 절 구분용으로 실측 2위인 1.5pt 유지).
- **📑 `□` 아래 `가./나.` 소제목 들여쓰기**: h3·h4 문단에 왼쪽 여백이 없어, `□`가 내어쓰기로 부호를 왼쪽에 내미는 동안 소제목이 대항목보다 **왼쪽**에 놓이며 계층이 사라졌습니다(실렌더 확인). `□` 부호폭만큼 들여써 대항목 제목 글자와 맞춥니다.
- **📎 붙임 항목 번호 유실 수정**: `붙임  1. …` 다음 줄의 들여쓴 `2. …`가 마크다운 리스트로 잡혀 항목부호(`ㆍ`)로 치환되며 **번호가 사라지던** 결함을 고쳤습니다. 붙임 전용 블록으로 처리해 실측 관행을 따릅니다 — 앞에 빈 줄 한 줄(실측 343건 중 95%), 번호 `1. 2. 3.` 유지(부호 치환 실측 0건), 둘째 항목 선행 공백 보존으로 첫 번호와 세로 정렬(실측 123건 중 122건).
- **✍️ 개조식 문체 검수 (`kordoc lint --munche`)**: 표기법과 축이 다른 **문체** 12룰을 추가했습니다 — 서술형 `~다` 종결(실측 0%), `~해야 한다` 당위, `A가 아니라 B(추상어)` 수사(실측 0건), 물음표·느낌표(본문 0건), 대구 슬로건, 항목 70자·결론 60자 초과, 리드문 `~하고자 함.`. 보고서·계획서·개조식 프리셋의 `generate` 경고 채널에 자동으로 붙고(생성은 막지 않음), `kordoc lint` 에서는 `--munche` 로 옵트인입니다. 기안문(경어 종결)·통지·회의록·보도자료는 문체 관행이 달라 적용하지 않습니다. 실측 근거와 규칙표는 [`docs/gaejosik-munche.md`](docs/gaejosik-munche.md). 통계 출처는 [jkf87/hwpx-skill](https://github.com/jkf87/hwpx-skill) v1.17.0 의 실무부서 보고서 15건(2,288줄) 실측입니다.

## v4.9.0 변경사항

- **🧹 공문서 AI 슬롭 검수 (lint `AI_*` 룰)**: 생성형 AI 초안이 공문서로 흘러들 때 남는 문체 흔적을 표기법 검수에 추가했습니다 — 줄표 사용(`AI_EM_DASH`: — – ―), 한 줄 강조(볼드) 3회 이상(`AI_BOLD_OVERUSE`). `kordoc lint` 와 `generate` 경고 채널에 공통 적용되는 조언용 warning 이며, 편람 13룰 원전과 구분되는 kordoc 자체 룰입니다. 실제 자치구 검토보고서를 공문서 HWPX 로 뽑는 작업에서 역도출했습니다. 작성 지침은 `.claude/skills/gongmunseo/`(줄표→쉼표·괄호, 강조는 핵심 한 곳, 외래어→행정용어)에 함께 반영.
- **📄 렌더러가 명시적 쪽 나눔(`pageBreak="1"`)을 존중**: 한컴 '쪽 나누기'로 저장되는 `<hp:p pageBreak="1">` 속성을 reflow 렌더가 무시하고 이어 그리던 것을 수정 — 이제 해당 문단부터 새 페이지로 조판합니다. 개조식 프리셋의 장 배너, 후처리로 쪽을 나눈 생성본이 한컴과 같은 페이지 경계로 렌더됩니다.
