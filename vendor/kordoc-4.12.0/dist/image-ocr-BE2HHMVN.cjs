...
var _chunkHFJWCAJ2cjs = require('./chunk-HFJWCAJ2.cjs');
require('./chunk-TROD36DK.cjs');
require('./chunk-AT23GFSK.cjs');
...
// src/ocr/image-ocr.ts
var IMAGE_SCALE = 3;
async function parseImageDocument(buffer, options) {
  const warnings = [];
  if (typeof _optionalChain([options, 'optionalAccess', _ => _.ocr]) === "function") {
  ...
  await _chunkHFJWCAJ2cjs.ensureOcrModels.call(void 0, (p) => {
    if (p.phase === "download" && p.downloaded === 0) {
      process.stderr.write(`[kordoc-ocr] ${p.spec.name} \uB2E4\uC6B4\uB85C\uB4DC \uC911 (~${p.spec.sizeMb}MB)...
`);
    }
  });
  const engine = await _chunkHFJWCAJ2cjs.getOcrEngine.call(void 0, );
  ...
  const ruling = _chunkHFJWCAJ2cjs.detectRulingLines.call(void 0, data, width, height, IMAGE_SCALE);
  const extraLines = _chunkHFJWCAJ2cjs.rulingToPdfLines.call(void 0, ruling, IMAGE_SCALE, pdfH);
  return { blocks: _chunkHFJWCAJ2cjs.ocrItemsToBlocks.call(void 0, items, 1, pdfW, pdfH, IMAGE_SCALE, extraLines, _optionalChain([options, 'optionalAccess', _2 => _2.tables]) !== false), warnings };
}
async function decodeToRgba(buffer) {
  let sharp;
  try {
    const mod = await Promise.resolve().then(() => _interopRequireWildcard(require("sharp")));
    sharp = typeof mod === "function" ? mod : _nullishCoalesce(mod.default, () => ( mod));
  } catch (e) {
    throw new (0, _chunkBQSWSMRGcjs.KordocError)(
      `\uC774\uBBF8\uC9C0 \uD30C\uC2F1\uC5D0\uB294 optional dependency 'sharp' \uAC00 \uD544\uC694\uD569\uB2C8\uB2E4. \`npm install sharp\` \uD6C4 \uB2E4\uC2DC \uC2E4\uD589\uD558\uC138\uC694. \uC6D0\uC778: ${e.message}`
    );
  }
...
exports.parseImageDocument = parseImageDocument;
//# sourceMappingURL=image-ocr-BE2HHMVN.cjs.map