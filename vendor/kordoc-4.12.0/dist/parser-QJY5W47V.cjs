...
var _chunkAT23GFSKcjs = require('./chunk-AT23GFSK.cjs');
require('./chunk-T7GRHAIW.cjs');
    ...
    try {
      store.get(objId, (data) => {
        if (!done) {
          ...
          clearTimeout(timer);
          resolve(_nullishCoalesce(data, () => ( null)));
        }
      });
    } catch (e2) {
      if (!done) {
...
function toRgba(img) {
  ...
  if (kind === _pdfmjs.ImageKind.RGBA_32BPP) {
    if (data.length < w * h * 4) return null;
    rgba.set(data.subarray(0, w * h * 4));
    return rgba;
  }
  if (kind === _pdfmjs.ImageKind.RGB_24BPP) {
  ...
  if (kind === _pdfmjs.ImageKind.GRAYSCALE_1BPP) {
    const stride = w + 7 >> 3;
...
async function extractPageImages(page, fnArray, argsArray, pageNumber, state, warnings) {
  ...
  for (let i = 0; i < fnArray.length; i++) {
    ...
    let imgData = null;
    let dedupeId = null;
    if (op === _pdfmjs.OPS.paintImageXObject || op === _pdfmjs.OPS.paintImageXObjectRepeat) {
      const objId = _optionalChain([argsArray, 'access', _2 => _2[i], 'optionalAccess', _3 => _3[0]]);
      if (typeof objId !== "string" || pageSeenIds.has(objId)) continue;
      pageSeenIds.add(objId);
      dedupeId = objId;
    } else if (op !== _pdfmjs.OPS.paintInlineImageXObject) {
      continue;
    }
    ...
    if (dedupeId) {
      ...
      imgData = _optionalChain([argsArray, 'access', _4 => _4[i], 'optionalAccess', _5 => _5[0]]);
    }
    if (!_optionalChain([imgData, 'optionalAccess', _6 => _6.data]) || !imgData.width || !imgData.height) continue;
    ...
    const hash = `${w}x${h}k${_nullishCoalesce(imgData.kind, () => ( "?"))}h${fnv1a(imgData.data)}${dedupeId ? "" : "inline"}`;
    if (state.seen.has(hash)) continue;
    const rgba = toRgba(imgData);
    if (!rgba) continue;
    const png = _chunkTYYJSOZScjs.encodePng.call(void 0, w, h, rgba);
    state.imageIndex++;
...
function cleanPdfText(text) {
  return mergeKoreanLines(
    ...
    return _chunkAT23GFSKcjs.collapseEvenSpacing.call(void 0, line);
...
function applyLinkAnnotations(items, annots) {
  ...
  for (const a of annots) {
    ...
    const url = _chunkBQSWSMRGcjs.sanitizeHref.call(void 0, a.url);
    if (!url) continue;
...
async function applyFormulaOcr(buffer, blocks, pageFilter, effectivePageCount, warnings, _onProgress) {
  const formulaMod = await Promise.resolve().then(() => _interopRequireWildcard(require("./formula-YMGXXRBQ.cjs")));
...
`);
  ...
  try {
    const effectiveFilter = _nullishCoalesce(pageFilter, () => ( new Set(Array.from({ length: effectivePageCount }, (_, i) => i + 1))));
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          loadingTask.destroy();
          reject(new (0, _chunkBQSWSMRGcjs.KordocError)("PDF \uB85C\uB529 \uD0C0\uC784\uC544\uC6C3 (30\uCD08 \uCD08\uACFC)"));
        }, PDF_LOAD_TIMEOUT_MS);
      })
    ]);
  } finally {
...
async function parsePdfDocument(buffer, options) {
  const formulaBuffer = _optionalChain([options, 'optionalAccess', _7 => _7.formulaOcr]) ? buffer.slice(0) : null;
  const ocrBuffer = _optionalChain([options, 'optionalAccess', _8 => _8.ocr]) ? buffer.slice(0) : null;
  const doc = await loadPdfWithTimeout(buffer);
  try {
    const pageCount = doc.numPages;
    if (pageCount === 0) throw new (0, _chunkBQSWSMRGcjs.KordocError)("PDF\uC5D0 \uD398\uC774\uC9C0\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4.");
    ...
    const pageFilter = _optionalChain([options, 'optionalAccess', _9 => _9.pages]) ? _chunkDCZVOIEOcjs.parsePageRange.call(void 0, options.pages, effectivePageCount) : null;
    ...
    for (let i = 1; i <= effectivePageCount; i++) {
      ...
      try {
        ...
        const items = _chunkAT23GFSKcjs.normalizeItems.call(void 0, rawItems);
        const { visible, hiddenCount } = _chunkAT23GFSKcjs.filterHiddenText.call(void 0, items, viewport.width, viewport.height);
        if (hiddenCount > 0) {
        ...
        if (pageArea > 0) {
          const imageRegions = _chunkAT23GFSKcjs.extractImageRegions.call(void 0, opList.fnArray, opList.argsArray);
          let uncovered = 0;
        ...
        const pageBlocks = _chunkAT23GFSKcjs.extractPageBlocksWithLines.call(void 0, visible, i, opList, viewport.width, viewport.height, void 0, _optionalChain([options, 'optionalAccess', _10 => _10.tables]) !== false);
        ...
        if (totalTextBytes > MAX_TOTAL_TEXT) throw new (0, _chunkBQSWSMRGcjs.KordocError)("\uD14D\uC2A4\uD2B8 \uCD94\uCD9C \uD06C\uAE30 \uCD08\uACFC");
        parsedPages++;
        _optionalChain([options, 'optionalAccess', _11 => _11.onProgress, 'optionalCall', _12 => _12(parsedPages, totalTarget)]);
      } catch (pageErr) {
        if (pageErr instanceof _chunkBQSWSMRGcjs.KordocError) throw pageErr;
    ...
    if (_optionalChain([options, 'optionalAccess', _13 => _13.ocr]) && ocrBuffer) {
      ...
      if (targets.size > 0) {
        try {
          const { runPdfOcr } = await Promise.resolve().then(() => _interopRequireWildcard(require("./pdf-ocr-T3RHIZKY.cjs")));
          ...
          if (ocrPageBlocks.size > 0) {
            ...
            merged.sort((a, b) => (_nullishCoalesce(a.pageNumber, () => ( 0))) - (_nullishCoalesce(b.pageNumber, () => ( 0))));
            blocks.length = 0;
            blocks.push(...merged);
    ...
    if (_optionalChain([options, 'optionalAccess', _14 => _14.removeHeaderFooter]) !== false && parsedPageCount >= 3) {
      const removed = _chunkAT23GFSKcjs.removeHeaderFooterBlocks.call(void 0, blocks, pageHeights, warnings);
      for (let ri = removed.length - 1; ri >= 0; ri--) {
        blocks.splice(removed[ri], 1);
      }
    }
    _chunkAT23GFSKcjs.mergeCrossPageTables.call(void 0, blocks);
    injectPageImageBlocks(blocks, pageImageBlocks);
    if (_optionalChain([options, 'optionalAccess', _15 => _15.formulaOcr]) && formulaBuffer) {
      try {
    ...
    const medianFontSize = _chunkAT23GFSKcjs.computeMedianFontSizeFromFreq.call(void 0, fontSizeFreq);
    if (medianFontSize > 0) {
      _chunkAT23GFSKcjs.detectHeadings.call(void 0, blocks, medianFontSize);
    }
    _chunkAT23GFSKcjs.detectMarkerHeadings.call(void 0, blocks);
    _chunkAT23GFSKcjs.detectTableCaptions.call(void 0, blocks);
    _chunkAT23GFSKcjs.detectKoreanListBlocks.call(void 0, blocks);
    const outline = blocks.filter((b) => b.type === "heading" && b.level && b.text).map((b) => ({ level: b.level, text: b.text, pageNumber: b.pageNumber }));
    sanitizeBlockControlChars(blocks);
    let markdown = cleanPdfText(_chunkTYYJSOZScjs.blocksToMarkdown.call(void 0, blocks));
    return {
      markdown,
      blocks,
      metadata,
...
async function extractPdfMetadata(doc, metadata) {
  try {
    ...
    if (!_optionalChain([result, 'optionalAccess', _16 => _16.info])) return;
...
exports.cleanPdfText = cleanPdfText; exports.detectKoreanListBlocks = _chunkAT23GFSKcjs.detectKoreanListBlocks; exports.detectTableCaptions = _chunkAT23GFSKcjs.detectTableCaptions; exports.extractPdfMetadataOnly = extractPdfMetadataOnly; exports.mergeCrossPageTables = _chunkAT23GFSKcjs.mergeCrossPageTables; exports.parsePdfDocument = parsePdfDocument; exports.removeHeaderFooterBlocks = _chunkAT23GFSKcjs.removeHeaderFooterBlocks;
//# sourceMappingURL=parser-QJY5W47V.cjs.map