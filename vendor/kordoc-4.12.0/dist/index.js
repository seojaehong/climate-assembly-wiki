...
function createXmlParser(warnings) {
  return new DOMParser({
    onError(level, msg2) {
      ...
      warnings?.push({ code: "MALFORMED_XML", message: `XML ${level === "warning" ? "\uACBD\uACE0" : "\uC624\uB958"}: ${msg2}` });
    }
  });
}
...
var attrOf = (xml, name) => xml.match(new RegExp(`${name}="([^"]*)"`))?.[1];
...
function decryptEntry(cipher, entry, startKey) {
  ...
  for (const prf of ["sha1", "sha256"]) {
    try {
      ...
      const plain2 = inflateRawSync(raw, { maxOutputLength: MAX_DECOMPRESS_SIZE });
      const sum = createHash("sha256").update(plain2.subarray(0, Math.min(CHECKSUM_PREFIX_BYTES, plain2.length))).digest();
      if (sum.length === entry.checksum.length && timingSafeEqual(sum, entry.checksum)) return plain2;
    } catch {
    }
  }
  return void 0;
...
function buildTableWithCellMeta(state, keepAnchoredEmptyCols, keepEmptyParagraphs) {
  const table2 = buildTable(state.rows, { keepAnchoredEmptyCols, keepEmptyParagraphs });
  if (state.caption) table2.caption = state.caption;
  if (state.captionBlocks?.length) table2.captionBlocks = state.captionBlocks;
  const anchors = [];
  {
  ...
  for (const row of state.rows) {
    for (const src of row) {
      ...
      if (src.rowAddr !== void 0 && src.colAddr !== void 0) {
        const cand = table2.cells[src.rowAddr]?.[src.colAddr];
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (tag === "subList" || tag === "p" || tag === "para") {
      if (tag === "subList") {
        ...
        if (text) {
          if (ph?.prefix) text = ph.prefix + " " + text;
          const block = { type: "paragraph", text, style: info.style ?? void 0, pageNumber: ctx.page };
...
function desEcbDecrypt(data, key) {
  const decipher = createDecipheriv2("des-ede3-ecb", Buffer.concat([key, key, key]), null);
  decipher.setAutoPadding(false);
...
function runToLatex(r) {
  let out = "";
  for (const t of childrenByLocalName(r, "t")) out += t.textContent ?? "";
  return out;
}
...
function nodeToLatex(el) {
  ...
  switch (tag) {
    ...
    case "groupChr":
      return childrenToLatex(findChildByLocalName(el, "e") ?? el);
...
function buildHmlTable(el, warnings, keep, tableDepth) {
  ...
  if (structured.length > 0) {
    ...
    for (const { ctx, cellBlocks } of structured) {
      let target = table2.cells[ctx.rowAddr ?? -1]?.[ctx.colAddr ?? -1];
  ...
  if (table2.cols >= 2) {
    for (let r = 0; r < table2.rows; r++) {
      for (let c = 0; c < table2.cols - 1; c++) {
        ...
        const valueCell = table2.cells[r]?.[c + 1];
    ...
    if (allLabels) {
      for (let r = 1; r < table2.rows; r++) {
        for (let c = 0; c < table2.cols; c++) {
          ...
          const value = table2.cells[r]?.[c]?.text.trim() ?? "";
          if (label && value) {
...
function collectIRTables(blocks, depth) {
  ...
  for (const block of blocks) {
    ...
    for (const row of block.table.cells) {
      for (const cell2 of row) {
        if (cell2?.blocks?.length) out.push(...collectIRTables(cell2.blocks, depth + 1));
      }
    }
  }
  return out;
  ...
  if (table2.rows >= 2 && table2.cols >= 2) {
    ...
    for (let r = 1; r < table2.rows; r++) {
      for (let c = 0; c < table2.cols; c++) {
        ...
        const valueCell = table2.cells[r]?.[c];
...
function scanClickHereRegions(xml) {
  ...
  while ((m = FIELD_BEGIN_RE.exec(xml)) !== null) {
    ...
    if ((attrOf2(attrs, "type") ?? "").toUpperCase() !== "CLICK_HERE") continue;
...
async function fillHwpx(hwpxBuffer, values, blockedLabels) {
  ...
  precheckZipSize(hwpxBuffer);
  const zip = await JSZip7.loadAsync(hwpxBuffer);
  const sectionPaths = Object.keys(zip.files).filter((name) => /[Ss]ection\d+\.xml$/i.test(name)).sort();
...
function borderFillEntry(id, b, fill) {
  const side = (name, v) => v ? `        <hh:${name} type="${v[2] ?? "SOLID"}" width="${v[0]}" color="${v[1]}"/>` : `        <hh:${name} type="NONE" width="0.1 mm" color="#000000"/>`;
  let brush = "";
  ...
  if (inlineDoc) {
    ...
    for (const m of text.matchAll(re)) {
      ...
      if (m[1] !== void 0) {
        ...
        const href = sanitizeHref(m[3].trim()) ?? void 0;
...
function estimateRowHeight(cells, widths, charHeight) {
  ...
  cells.forEach((cell2, c) => {
    const usable = Math.max((widths[c] ?? widths[widths.length - 1]) - CELL_PAD, 1e3);
    let lines = 0;
...
function generateTable(rows, theme, style = null, remap = null, seq = 0, images = null) {
  ...
  const trElements = rows.map((row, rowIdx) => {
    ...
    const tdElements = cells.map((cell2, colIdx) => {
      ...
      const bf = prof?.cellBf.get(k) ?? (reg ? reg.get(dataCellSpec({
        ...
        colCnt,
        headerRows: 1,
        fill: isHeaderRow ? style.headerFill ?? "#E6E6E6" : isLabelCell2 ? style.labelFill ?? "#E7E7E7" : void 0
  ...
  const hostPr = reg && style?.rightParaPr !== void 0 ? style.rightParaPr : 0;
...
async function markdownToHwpx(markdown, options) {
  ...
  try {
    ...
    const chartParts = [];
    const supplied = options?.images ? new Map(Object.entries(options.images).map(([k, v]) => [k, v instanceof Uint8Array ? v : new Uint8Array(v)])) : void 0;
...
function anchorRunInfo(xml, para2, anchor) {
  ...
  if (!tr) {
    const head = anchor[0];
    tr = para2.tRanges.find((r) => !r.selfClosing && xml.slice(r.contentStart, r.contentEnd).includes(head)) ?? para2.tRanges[para2.tRanges.length - 1];
  }
  if (!tr) return null;
...
function cellContentWidthMm(xml, cell2) {
  ...
  if (mg) {
    content -= Number(attrOf3(mg[0], "left") ?? 0) + Number(attrOf3(mg[0], "right") ?? 0);
  }
...
function cellLeftOffsetMm(xml, table2, cell2) {
  ...
  let sum = 0;
  for (let col = 0; col < cell2.colAddr; col++) sum += colW.get(col) ?? 0;
...
async function placeSealHwpx(hwpxBuffer, ops) {
  ...
  const u8 = new Uint8Array(hwpxBuffer);
  precheckZipSize(hwpxBuffer);
  const zip = await JSZip9.loadAsync(hwpxBuffer);
  ...
  for (const op of ops) {
    ...
    const paraPrId = attrOf3(paraOpenTag(xml, site.para), "paraPrIDRef") ?? "0";
...
function parseTable3(tbl2, tableIndex, borderFills, charPrs) {
  const rows = num(tbl2.getAttribute("rowCnt")) ?? 0;
  const cols = num(tbl2.getAttribute("colCnt")) ?? 0;
...
async function hwpxToProfile(input) {
  ...
  const zip = await JSZip10.loadAsync(buf);
  const parser = createXmlParser();
...
function patchNestedTables(table2, scanTable, oc, origTables, editedTables, ctx, skip, depth) {
  const irCell = table2.cells[oc.gridR]?.[oc.gridC];
...
function applyCellEdit(table2, scanTable, gridR, gridC, newLines, ctx, before, after, origLineCount) {
  ...
  const irCell = table2.cells[gridR]?.[gridC];
...
async function patchHwpx(original, editedMarkdown, options) {
  ...
  let zip;
  try {
    zip = await JSZip11.loadAsync(original);
  } catch {
...
function convertParagraphToTable(block, orig, edited, ctx, skip) {
  ...
  const outerParaPrId = extractParaPrIdRef(scan.xml, para2.start) ?? 0;
...
function compressWithTail(raw) {
  ...
  return Buffer.concat([deflateRawSync2(raw), tail]);
}
...
async function validateHwpx(buffer) {
  ...
  let zip;
  try {
    zip = await JSZip12.loadAsync(buffer);
  } catch (err) {
    return {
      ok: false,
...
async function buildState(bytes) {
  ...
  const zip = await JSZip13.loadAsync(bytes);
  const sectionPaths = await resolveSectionEntryNames(zip);
  if (sectionPaths.length === 0) {
...
var HwpxSession = class _HwpxSession {
  ...
  sourceRef(blockIndex) {
    ...
    if (block.type === "paragraph" || block.type === "heading") {
      const para2 = st.paraMap.get(blockIndex)?.para;
...
function drawTable(tbl2, tx, ty, ctx, depth) {
  ...
  for (const g of geom) {
    ...
    if (bf?.fill) emit(ctx, `<rect x="${pt(g.x)}" y="${pt(g.y)}" width="${pt(g.w)}" height="${pt(g.h)}" fill="${escapeXml3(bf.fill)}"/>`);
  }
  for (const g of geom) {
    const { c } = g;
    if (!c.sub) continue;
...
async function renderHwpxToSvg(input, options) {
  ...
  let zip;
  try {
    zip = await JSZip14.loadAsync(input);
  } catch {
...
async function parseImage(buffer, options) {
  try {
    const { parseImageDocument } = await import("./image-ocr-MBWZK3ZT.js");
...
async function parseHwpx(buffer, options) {
  try {
    ...
    return { success: true, fileType: "hwpx", markdown, blocks, metadata, outline, warnings, images: images?.length ? images : void 0, pageCount: metadata?.pageCount };
  } catch (err) {
...
async function parseHwp(buffer, options) {
  try {
    ...
    return { success: true, fileType: "hwp", markdown, blocks, metadata, outline, warnings, images: images?.length ? images : void 0, pageCount: metadata?.pageCount };
  } catch (err) {
...
async function parsePdf(buffer, options) {
  ...
  try {
    const mod = await import("./parser-CMNHBVTL.js");
...
async function parseDocx(buffer, options) {
  try {
    ...
    return { success: true, fileType: "docx", markdown, blocks, metadata, outline, warnings, images: images?.length ? images : void 0, pageCount: metadata?.pageCount };
  } catch (err) {
